...
class InternalRmatics(BaseService):
    ...
    def send_submit(self, file, user_id: int, problem_id: int, statement_id: int, lang_id: int):
        data = {
            'lang_id': lang_id,
            'user_id': user_id,
        ...
        content = self.client.post_data(url, json=data, files={'file': file}, silent=True)
        return content

    def get_runs_filter(self, problem_id: int, args: dict, is_admin=False):
        filter_args = {
            **args,
            'is_admin': is_admin,
        }
        url = self.service_url + f'/problem/{problem_id}/submissions/'
        content = self.client.get_data(url, params=filter_args, silent=True, default=[])
